...
<body class="body-content" style="background-image: url('images/appointment-page.jpg');background-position:right; background-repeat: no-repeat;background-attachment: fixed;  background-size: cover;" >
  ...
  $em=$_SESSION["email"];
   ...
   while($row=mysqli_fetch_row($rs)){
    ...
    ?></a></li>
        <li><a href="#" onclick="toggleModal();">LogOut<i class="material-icons left">person</i></a></li>
      </ul>
      </div>
    </nav>
    </div>
    <ul class="sidenav" id="mobile-demo">
      <li><a href="doctor.php">Dashboard</a>
      <li><a href="#">Appointments</a></li>
      <li><a href="#"><?php
  $em=$_SESSION["email"];
   $sql = "select fname from doctorinfo where doc_email = '$em'";  
...
<div class="wrapper">
    <section class="users">
      <header>
        <div class="content">
          <?php 
            $sql ="select *from doctorinfo where doc_email = '$em'";
            $rs=mysqli_query($conn, $sql);
          ...
          ?>
          <div class="details ">
            <span class="col s6 offset-s1"><i class="material-icons down">person</i><?php echo "Dr.". $row['fname']. " " . $row['lname'] ?></span>  
          <hr>
        </div>
      </header>
      <hr>
<!--list of appointments-->  
      <div class="users-list">
                  <?php
            $outgoing_id = $row['doc_email'];
            $sql3 = "SELECT * FROM appointment_info WHERE doc_email = '$em' ORDER BY ap_date DESC";
            ...
            if(mysqli_num_rows($query) == 0){
                $output .= "No appointment today";
                                    ...
                                    while($row = mysqli_fetch_assoc($query)){
                                        ...
                                        $sql4 = "SELECT * FROM patientinfo WHERE pat_email = '$incoming'";
                                        $qe=mysqli_query($conn,$sql4);
                                        $r=mysqli_fetch_assoc($qe);
                                        $output .= '<a href="chat.php?user_id='. $r['pat_email'] .'">
                                                    <div class="content">
